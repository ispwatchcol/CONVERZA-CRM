...
const whatsappStatusLabel = computed(() => whatsappConnected.value ? 'Conectado' : 'Desconectado');
...
const navItems = [
    ...
    { name: 'Manual', route: 'manual.index', icon: 'manual' },
    // Soporte con NOSOTROS (no con los clientes del ISP): es la relación comercial
    // del workspace, así que solo la ve el admin. La ruta también está cerrada con
    // 'role:admin' — esto es la cortesía visual, no el control.
    { name: 'Soporte', route: 'support.index', icon: 'support', adminOnly: true },
...
const isTenantAdmin = computed(() => authUser.value?.role === 'admin');

// Los items marcados adminOnly no se le pintan al asesor. La autorización real
// vive en las rutas; esto solo evita ofrecerle un enlace que le daría 403.
const visibleNavItems = computed(() => navItems.filter(i => !i.adminOnly || isTenantAdmin.value));
...
<template>
    ...
    <div v-else class="h-screen flex overflow-hidden bg-gray-100">
        ...
        <svg v-else-if="item.icon === 'campaigns'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg>
                        ...
                        <svg v-else-if="item.icon === 'manual'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg>
                        <svg v-else-if="item.icon === 'support'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" /></svg>
            ...
            <div class="mb-3 transition-all duration-300" :class="sidebarOpen ? 'p-3 mx-3 rounded-xl bg-sidebar-hover border border-white/5' : 'flex justify-center'">
                <div class="flex items-center" :class="sidebarOpen ? 'space-x-3' : 'flex-col'">
                    <div
                        class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 relative"
                        :class="whatsappConnected ? 'bg-accent/20' : 'bg-red-500/20'"
                        :title="!sidebarOpen ? `WhatsApp API: ${whatsappStatusLabel}` : ''"
                    >
                        <svg class="w-4 h-4" :class="whatsappConnected ? 'text-accent' : 'text-red-400'" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                        <span
                            class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full ring-2 ring-sidebar"
                            :class="whatsappConnected ? 'bg-accent' : 'bg-red-500'"
                        ></span>
                    </div>
                    <div v-show="sidebarOpen" class="truncate">
                        <p class="text-xs text-white font-medium">WhatsApp API</p>
                        <p class="text-[10px]" :class="whatsappConnected ? 'text-accent' : 'text-red-400'">{{ whatsappStatusLabel }}</p>
                    </div>
                </div>
            </div>
        </aside>
